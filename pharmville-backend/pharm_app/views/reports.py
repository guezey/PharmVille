...
def report_with_date(pharm_id: int, start_date: datetime.date, end_date: datetime.date):
    ...
    cursor.execute("""SELECT round(SUM(unit_price * count), 2 )  AS revenue
            FROM Orders
    ...
    report['revenue'] = revenue if revenue else 0
    cursor.execute("""SELECT COUNT(o.order_id) AS order_count
                            FROM Orders AS o
                            WHERE o.pharmacy_id = %s AND order_time BETWEEN %s AND %s
                            """, (pharm_id, start_date, end_date))
    ...
    cursor.execute("""SELECT COUNT(o.order_id) AS canceled_orders_count 
                            ...
                            WHERE o.order_status = 'CANCELED'
                                AND o.pharmacy_id = %s AND order_time BETWEEN %s AND %s
        """, (pharm_id, start_date, end_date))
